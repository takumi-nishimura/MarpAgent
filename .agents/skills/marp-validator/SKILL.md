...
`-v` is the single validation command; `--lint` remains only as a deprecated alias. `--autofix` applies the safe typography fixes (lifting `.text-xs2`/`.text-xs3` to `text-sm`, unwrapping `<small>`) before validating; add `--dry-run` to print a unified diff on stderr without writing. Fenced code and inline code spans are never rewritten.
