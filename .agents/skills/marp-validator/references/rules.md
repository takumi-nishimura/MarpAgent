...
Bullet-like lines inside fenced code blocks (`` ``` `` or `~~~`) are not counted, and `-v --autofix` leaves fenced code and inline code spans byte-identical.
